 ...
 * final FunctionInfo.
 */
function createFunctionInfo(func, context, serialize) {
    ...
    function serializeWorker() {
        ...
        for (const keyOrSymbol of getOwnPropertyNamesAndSymbols(func)) {
            ...
            functionInfo.env.set(getOrCreateEntry(keyOrSymbol, undefined, context, serialize), { entry: getOrCreateEntry(funcProp, undefined, context, serialize) });
        }
        ...
        if (superEntry) {
            ...
            capturedValues.set(getOrCreateEntry("__super", undefined, context, serialize), { entry: superEntry });
        ...
        if (functionDeclarationName !== undefined) {
            capturedValues.set(getOrCreateEntry(functionDeclarationName, undefined, context, serialize), { entry: funcEntry });
        }
        return functionInfo;
 ...
 */
function getOrCreateEntry(obj, capturedObjectProperties, context, serialize) {
        ...
        if (!serialize(obj)) {
            ...
            for (const key of Object.getOwnPropertyNames(obj)) {
                if (key !== "length" && obj.hasOwnProperty(key)) {
                    entry.array[key] = getOrCreateEntry(obj[key], undefined, context, serialize);
                }
            }
        }
    ...
    // property filtering.
    function serializeObjectWorker(localObjectProperties) {
        ...
        if (localObjectProperties.length === 0) {
            ...
            for (const keyOrSymbol of ownPropertyNamesAndSymbols) {
                ...
                if (!environment.has(keyEntry)) {
                    ...
                    const valEntry = getOrCreateEntry(obj[keyOrSymbol], undefined, context, serialize);
            ...
            for (const capturedInfo of localObjectProperties) {
                ...
                if (!environment.has(keyEntry)) {
                    ...
                    if (!propertyInfo) {
                        ...
                        // undefined value.
                        const valEntry = getOrCreateEntry(objPropValue, undefined, context, serialize);
    ...
    function propInfoUsesNonLexicalThis(capturedInfo, propertyInfo, valEntry) {
    ...
    function getPropertyInfo(key) {
        for (let current = obj; current; current = Object.getPrototypeOf(current)) {
            ...
            if (desc) {
                ...
                if (desc.get) {
                    propertyInfo.get = getOrCreateEntry(desc.get, undefined, context, serialize);
                }
                if (desc.set) {
                    propertyInfo.set = getOrCreateEntry(desc.set, undefined, context, serialize);
                }
                return propertyInfo;
            }
        }
