 ...
 */
function createFunctionInfoAsync(func, serialize) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield processAsyncWorkQueue();
        return entry.function;
        function addGlobalInfo(key) {
        ...
        function addGeneratorEntries() {
            ...
            context.cache.set(Object.getPrototypeOf(emptyGenerator), { expr: "Object.getPrototypeOf(function*(){})" });
            context.cache.set(Object.getPrototypeOf(emptyGenerator.prototype), { expr: "Object.getPrototypeOf((function*(){}).prototype)" });
        }
        function processAsyncWorkQueue() {
            return __awaiter(this, void 0, void 0, function* () {
 ...
 * final FunctionInfo.
 */
function createFunctionInfo(func, context, serialize, logInfo) {
    // logInfo = logInfo || func.name === "addHandler";
    const file = v8.getFunctionFile(func);
    const line = v8.getFunctionLine(func);
    const column = v8.getFunctionColumn(func);
    ...
    function serializeWorker() {
        ...
        for (const keyOrSymbol of getOwnPropertyNamesAndSymbols(func)) {
            ...
            const funcProp = func[keyOrSymbol];
            ...
            functionInfo.env.set(getOrCreateEntry(keyOrSymbol, undefined, context, serialize, logInfo), { entry: getOrCreateEntry(funcProp, undefined, context, serialize, logInfo) });
        }
        ...
        if (superEntry) {
            ...
            capturedValues.set(getOrCreateEntry("__super", undefined, context, serialize, logInfo), { entry: superEntry });
        ...
        if (functionDeclarationName !== undefined) {
            capturedValues.set(getOrCreateEntry(functionDeclarationName, undefined, context, serialize, logInfo), { entry: funcEntry });
        }
        return functionInfo;
        function processCapturedVariables(capturedVariables, throwOnFailure) {
        ...
        function processCapturedVariable(capturedVariables, name, value) {
    ...
    function processDerivedClassConstructor(protoEntry) {
        ...
        for (const keyOrSymbol of getOwnPropertyNamesAndSymbols(func)) {
            if (keyOrSymbol !== "length" && keyOrSymbol !== "name" && keyOrSymbol !== "prototype") {
                // static method.
                const classProp = func[keyOrSymbol];
...
Consider using import('${moduleName}') or require('${moduleName}') inside ${location}`;
    ...
    throw new errors_1.RunError(message);
}
...
function getFunctionLocation(loc) {
    ...
    if (loc.file) {
        name += `: ${path_2.basename(loc.file)}(${loc.line + 1},${loc.column})`;
    }
...
function isDefaultFunctionPrototype(func, prototypeProp) {
 ...
 */
function getOrCreateEntry(obj, capturedObjectProperties, context, serialize, logInfo) {
    ...
    function dispatchAny() {
        if (!serialize(obj)) {
            ...
            context.asyncWorkQueue.push(() => __awaiter(this, void 0, void 0, function* () {
                const val = yield obj.promise();
            ...
            for (const key of Object.getOwnPropertyNames(obj)) {
                if (key !== "length" && obj.hasOwnProperty(key)) {
                    entry.array[key] = getOrCreateEntry(obj[key], undefined, context, serialize, logInfo);
                }
            }
        }
    ...
    // property filtering.
    function serializeObjectWorker(localCapturedPropertyChains) {
        const objectInfo = entry.object || { env: new Map() };
    ...
    function serializeAllObjectProperties(environment) {
        ...
        for (const keyOrSymbol of ownPropertyNamesAndSymbols) {
            ...
            if (!environment.has(keyEntry)) {
                environment.set(keyEntry, undefined);
                const propertyInfo = getPropertyInfo(obj, keyOrSymbol);
                if (!propertyInfo) {
                    throw new Error("Could not find property info for 'own' property.");
                }
                const valEntry = getOrCreateEntry(obj[keyOrSymbol], undefined, context, serialize, logInfo);
    ...
    function serializeSomeObjectProperties(environment, localCapturedPropertyChains) {
        ...
        for (const propName of propChainFirstNames) {
            ...
            if (!propertyInfo) {
                ...
                // undefined value.
                const valEntry = getOrCreateEntry(objPropValue, nestedPropChains, context, serialize, logInfo);
    ...
    function getPropertyInfo(on, key) {
        for (let current = on; current; current = Object.getPrototypeOf(current)) {
            const desc = Object.getOwnPropertyDescriptor(current, key);
            if (desc) {
                const propertyInfo = { hasValue: desc.value !== undefined };
                propertyInfo.configurable = desc.configurable;
                propertyInfo.enumerable = desc.enumerable;
                propertyInfo.writable = desc.writable;
                if (desc.get) {
                    propertyInfo.get = getOrCreateEntry(desc.get, undefined, context, serialize, logInfo);
                }
                if (desc.set) {
                    propertyInfo.set = getOrCreateEntry(desc.set, undefined, context, serialize, logInfo);
                }
                return propertyInfo;
            }
        }
...
function findModuleName(obj) {
    ...
    for (const path of Object.keys(require.cache)) {
        if (require.cache[path].exports === obj) {
            ...
            const modPath = path_1.relative(process.cwd(), path).replace(/\\/g, "\\\\");
            return "./" + modPath;
        }
    }
